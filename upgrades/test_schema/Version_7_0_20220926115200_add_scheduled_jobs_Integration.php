...
final class Version_7_0_20220926115200_add_scheduled_jobs_Integration extends TestCase
{
    use ExecuteMigrationTrait;

    private const string MIGRATION_LABEL = '_7_0_20220926115200_add_scheduled_jobs';
    ...
    {
        $this->deleteJobInstance('events_api_purge');
        $this->assertNull($this->jobInstanceId('events_api_purge'));
        $this->reExecuteMigration(self::MIGRATION_LABEL);
        $this->assertNotNull($this->jobInstanceId('events_api_purge'));
    }

    public function test_it_does_not_adds_an_instance_if_present()
    {
        $jobInstanceId = $this->jobInstanceId('events_api_purge');
        $this->assertNotNull($jobInstanceId);
        $this->reExecuteMigration(self::MIGRATION_LABEL);
        $this->assertEquals($jobInstanceId, $this->jobInstanceId('events_api_purge'));
    }
