...
class Version_7_0_20230201143400_drop_product_id_columns_and_clean_versioning_resource_uuid_columns_Integration extends TestCase
{
    ...
    public function test_it_empties_the_resource_uuid_when_the_version_is_not_product_related()
    {
        ...
        $productVersionId = $this->createProductRelatedVersionWithResourceUuid();
        $nonProductVersionId = $this->createNonProductRelatedVersionWithResourceUuid();

        $this->reExecuteMigration(self::MIGRATION_LABEL);

        $this->assertColumnsRemoved();
        $this->assertTriggersRemoved();
        $this->assertProductRelatedVersionHasResourceUuid($productVersionId);
...
SQL;
    ...
    private function assertProductRelatedVersionHasResourceUuid(int $productVersionId): void
    {
        $result = $this->fetchResourceIdForVersion($productVersionId);
        $this->assertNotNull($result);
    }

    private function assertNonProductRelatedVersionHasNoResourceUuid(int $nonProductVersionId): void
    {
        $result = $this->fetchResourceIdForVersion($nonProductVersionId);
